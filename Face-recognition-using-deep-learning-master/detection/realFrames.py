...
inputVideoReal = cv2.VideoCapture(os.path.join(dirname, 'videos/real.mp4'))
outputVideoReal = os.path.join(dirname, 'dataset/real')
...
while True:
	(grabbed, frame) = inputVideoReal.read()
	if not grabbed:
		break
	read += 1
	if read % 4 != 0:
	...
	if len(detections) > 0 :
		...
		if confidence > 0.5:
			...
			p = os.path.sep.join([outputVideoReal, "{}.png".format(saved)])
			cv2.imwrite(p, face)
			saved += 1
			print("[INFO] saved {} to disk".format(p))
inputVideoReal.release()
